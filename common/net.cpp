...
//=============================================================================
int Net::sendData(void *data, int &size, const char *remoteIP, const USHORT port) 
{
    int status;
    int sendSize = size;
    ...
    ret = sendto(sock, (const char*)data, sendSize, 0, (SOCKADDR *)&remoteAddr, sizeof(remoteAddr));
    if (ret == SOCKET_ERROR) 
    {
        status = WSAGetLastError();
        return ((status << 16) + NET_ERROR);
...
//=============================================================================
int Net::readData(void *data, int &size, char *senderIP, USHORT &port) 
{
    int status;
    int readSize = size;
    ...
    if(sock != NULL)
    {
        ...
        ret = recvfrom(sock, (char*)data, readSize, 0, (SOCKADDR *)&remoteAddr,
                       &remoteAddrSize);
        if (ret == SOCKET_ERROR) {
            status = WSAGetLastError();
            if ( status != WSAEWOULDBLOCK)  // don't report WOULDBLOCK error
