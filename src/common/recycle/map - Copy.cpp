...
inline std::vector<Space> sortByFCost(const std::vector<V3> vertexList, const CustomSet<Space>& spaceList)
{
...
std::vector<V3> Map::getVertexPathfind(const V3 & start, const V3 & end)
{
	...
	CustomSet<Node> openList, closedList;
	...
	{
		//Node& q = *new Node(sortByFCost({ start, end }, openList)[0]);
		Node q = Node(openList[0], { start, end });
		openList.erase(q);
		std::vector<Node> successors = *new std::vector<Node>(getAmbientFreeSpace(q, { start,end }));
	...
	if(done)
		while(1)
		{
			...
			if (endSuccessor.parent == nullptr)break;
...
std::vector<Node> Map::getAmbientFreeSpace(const Node& node, const std::vector<V3>& vertexList) const
{
	Node ambient[] = {
		...
		if (ambient[i].isValid())
		{
			if (vecAmbient.empty())
				...
				if (!vertexList.empty())
				{
					...
					{
						float fcost = ambient[i].getFCostForCenter(vertexList);
						if (vecAmbient[j].getFCostForCenter(vertexList)  > fcost)
						{
