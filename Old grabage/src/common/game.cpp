...
Game::Game() : m_hwnd(0), m_fps(0), m_initialized(false)
{
...
Game::~Game()
{
	m_pGraphics.reset(); // for release ImGui_d3d implement before imgui_win implement
}
...
LRESULT Game::messageHandler(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_ACTIVATEAPP:
		if (m_initialized)
			onWindowActivate(wParam > 0 ? true : false);
...
void Game::initialize(HINSTANCE hInstance, HWND hwnd)
{
	...
	CHECK_ERROR(m_pInput->initialize(m_hwnd), "Failed to initialize Input");
	CHECK_ERROR(m_pDx11Wrapper->initialize(this), "Failed to initialize Dx11Wrapper");
	CHECK_ERROR(m_pGraphics->initialize(this), "Failed to initialize Graphics");
...
void Game::onUpdateDisplay(bool saveSettings) const
{
...
bool Game::checkGameFiles() const
{
	...
	for (auto file : gameNS::gameFiles)
	{
		...
		{
			IMPL_WARNING(strFormat("File %s doesn't exist or is corrupted.", file.first.c_str()));
			return false;
		}
	}
...
void Game::onWindowActivate(bool activate)
{
	if (g_pGameSettings->windowed)
		return;

	if (activate)
		onUpdateDisplay(false);
	else
	{
		g_pGameSettings->windowed = true;
		onUpdateDisplay(false);
		g_pGameSettings->windowed = false;
	}
}
